...
public:
	...
	Pattern(b2World &world, sf::Texture* texture, b2BodyDef myBodyDef, b2FixtureDef fixtureDef,std::string patternFile) :
		AbstractSpawner(world, texture, myBodyDef, fixtureDef) {
		pugi::xml_document document;
		if (!document.load_file(patternFile.c_str()))
		{
			std::cout << "Error lors du loading de " << patternFile;
		}
		doc = document;
		createFromXml();
	...
	{
		return std::make_unique<Pattern>(*world, textureActuelle, myBodyDef, myFixtureDef,doc.name());
	};
